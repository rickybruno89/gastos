...
const calcItemsToPay = (paymentSourceBalance: PaymentSourceBalanceWithInclude[]) => {
  ...
  return totalsByName.filter((item) => item.totalAmount)
}
  ...
  return (
    <>
      <section className="hidden xl:block">
        <p className="font-bold py-1">Balance necesario en cuentas</p>
        <div className="flex flex-wrap gap-4">
          ...
          <div className="rounded-md bg-white p-4 md:p-6 w-full md:w-fit  min-w-[320px] flex flex-col ">
            ...
            {alreadyPaid.length ? (
              ...
              <span>No se pago nada</span>
            )}
          </div>
        </div>
      </section>
      <section className="block xl:hidden" id="source-balance-content">
                  ...
                  {paymentSourceItems.length ? (
                    <>
                      ...
                      <div className="flex justify-end items-center gap-6 mt-4 font-bold">
                        <span>TOTAL</span>
                        <span>
                          {formatCurrency(paymentSourceItems.reduce((acc, item) => acc + item.totalAmount, 0))}
                        </span>
                      </div>
                    </>
                  ) : (
